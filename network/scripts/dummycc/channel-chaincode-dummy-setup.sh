...
if [ $ORG_NUM == 1 ]; then
    ...
    peer chaincode install -n dummycc -v 5.1 -l node -p /opt/gopath/src/github.com/chaincode/dummycc
done
...
peer chaincode instantiate -o orderer.el-network.com:7050 -C $CHANNEL_NAME -n dummycc -l node -v 5.1 -c '{"Args":["initLedger"]}' -P "OR ('Org1MSP.member','Org2MSP.member','Org3MSP.member','Org4MSP.member')" --tls --cafile $CACERT_ORDERER
