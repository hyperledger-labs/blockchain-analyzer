...
if [ $ORG_NUM == 1 ]; then
    ...
    peer chaincode install -n applechain -v 1.5 -l node -p /opt/gopath/src/github.com/chaincode/applechain
done
...
peer chaincode instantiate -o orderer.el-network.com:7050 -C $CHANNEL_NAME -n applechain -l node -v 1.5 -c '{"Args":[]}' -P "OR ('Org1MSP.member','Org2MSP.member','Org3MSP.member','Org4MSP.member')" --tls --cafile $CACERT_ORDERER
